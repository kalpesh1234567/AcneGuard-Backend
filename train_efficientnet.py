"""
AcneGuard — EfficientNet-B0 Training Script
============================================
Trains an EfficientNet-B0 model (pretrained on ImageNet) on the acne severity dataset.
The output model file `acne_model.pth` is saved in the backend folder and is directly
compatible with vision.py.
...
# ─── Argument Parser ─────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Train AcneGuard EfficientNet-B0")
...
with open(LOG_FILE, "w", encoding="utf-8") as f:
    f.write("=== AcneGuard Training Progress ===\n")
...
# Training uses augmentation (flip, rotation, color jitter) to improve generalization.
...
if not os.path.exists(train_dir):
    ...
    print("Please organise your dataset as: dataset/train/<class>/ and dataset/val/<class>/")
    exit(1)
...
# --- Class Balance Handling (Crucial for Accurate Results) ---
# Count samples in each class to compute weights
class_counts = [len(os.listdir(os.path.join(train_dir, c))) for c in class_names]
total_samples = sum(class_counts)
# Inverse frequency weighting: weights[i] = total / (num_classes * count[i])
...
# Save class names to classes.json (must match vision.py expectations)
with open(CLASSES_FILE, "w") as f:
    json.dump(class_names, f)
log_print(f"Saved classes.json -> {CLASSES_FILE}")

# ─── Build EfficientNet-B0 (Transfer Learning) ────────────────────────────────
# Use ImageNet pretrained weights for the backbone, replace only the classifier head.
log_print("Loading EfficientNet-B0 with ImageNet pretrained weights...")
